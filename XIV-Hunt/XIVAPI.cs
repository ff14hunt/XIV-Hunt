...
namespace XIVAPI
{
    ...
    class XIVAPIClient
    {
        ...
        {
            HttpResponseMessage r = await GetAsync($"/search?string={WebUtility.UrlEncode(itemName)}&indexes=item&page={page}&string_algo=query_string&language={Thread.CurrentThread.CurrentUICulture.Name.Substring(0, 2)}");
